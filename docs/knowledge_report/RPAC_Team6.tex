...
{
    name=HTTP,
    description={Protocolo de Transferência de Hipertexto (do inglês \text{HyperText Transfer Protocol}).},
    text={HTTP},      
}
\newglossaryentry{REST}
{
    name=REST,
    description={Transferência de Estado Representacional (do inglês \text{Representational State Transfer}).},
    text={REST},      
}
...
\begin{document}
    ...
    A arquitetura do sistema é composta por um \textit{frontend} Web em React que recolhe os dados do paciente, apresenta o estado do processo e devolve conclusões e explicações, comunicando com o servidor por \glspl{HTTP}.
    No \textit{backend}, existem dois subsistemas periciais expostos por \glspl{API} independentes: 
    \begin{itemize}
        \item Um sistema pericial em Prolog, que utiliza bibliotecas \glspl{HTTP} para receber e enviar JSON e executar inferência declarativa.
        \item Um sistema pericial em Java com Spring Boot, que expõe serviços \glspl{REST} e utiliza o motor de regras Drools para avaliação de regras e geração de justificações.
    \end{itemize}
    
    \begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{./resources/pdf/arquitetura.jpg}
    \caption{Arquitetura lógica do sistema: React Web-App (cliente), \glspl{API}s de Prolog e Spring Boot (servidor) e Docker.}
